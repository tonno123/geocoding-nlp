        ...
        for sentence_n in range(total_sentence_n):
            ...
            for span_n in range(total_span_n):
                for token in input_data[article_n][sentence_n][span_n]:
                    ...
                    elif token.text[:3] == 'LOC':
                        spanlist.append(token)
            loccounter = -1
            prev_word_type = ''
            spanlist2 = []
            for word in list(reversed(spanlist)):
                if loccounter < 0 and not word.text[:3] == 'LOC':
                    continue
                elif word.pos_ != 'ADP' and word.text[:3] == 'LOC':
                    if not prev_word_type == 'LOC':
                        loccounter = loccounter + 1
                        spanlist2.append([[],[]])
                    spanlist2[loccounter][1].insert(0, word.text[3:].replace("_", " "))
                    prev_word_type = 'LOC'
